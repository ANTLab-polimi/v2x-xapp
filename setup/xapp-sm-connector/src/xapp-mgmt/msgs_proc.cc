...
void XappMsgHandler::operator()(rmr_mbuf_t *message, bool *resend){
	...
	// mdclog_write(MDCLOG_WARN, "Received RIC message");
...
void process_ric_indication(int message_type, transaction_identifier id, const void *message_payload, size_t message_len) {
	// mdclog_write(MDCLOG_INFO,"RIC indication with size: %ld",message_len);
	// mdclog_write(MDCLOG_WARN,"RIC indication with size: %ld",message_len);
	// std::cout << "RIC indication with size " << message_len << std::endl;

	std::string agent_ip = find_agent_ip_from_gnb(id); 
	// mdclog_write(MDCLOG_INFO,"Sending data to agent %s ", agent_ip.c_str());
	// mdclog_write(MDCLOG_WARN,"Sending data to agent %s ", agent_ip.c_str());

	mdclog_write(MDCLOG_WARN,"RIC indication with size: %ld to agent: %s", message_len, agent_ip.c_str());
